...
upload: 
	...
	@sleep 0.2
	@echo -n PIP...
	@echo "A:PIP I:U.U=CON:" > /dev/ttyUSB0
	@sleep 0.2
	@echo -n UPLOAD ---%
	@$(eval LINES=$(shell cat U.U | wc -l))
	@cnt=0;while IFS= read -r line;do cnt=$$(( cnt+100)); printf "\b\b\b\b%3d%%" $$(( $$cnt/$(LINES) )); echo "$$line">/dev/ttyUSB0;sleep 0.01; done < U.U
	@echo -n " "
	@echo -e "\x1A" > /dev/ttyUSB0
	@$(SERIAL) -load cpm8266 -sercfg $(EMULATIONBAUD)
